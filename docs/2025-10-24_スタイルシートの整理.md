...
- [x] 11. チャンネル・アイテムコンポーネント (`.channel-and-items-component-*`)
- [x] 12. 未読リスト (`.unreads-list`, `.unreads-nav`, `.unreads-channel-group-tab`)
- [x] 13. 足跡機能 (`.pawprint-*`, `.chase-paw-*`)
- [x] 14. プロフィール (`.profile`, `.profile-image`)
- [x] 15. オーディオプレイヤー (`.audio-player`, `.audio-player-on-the-image`)
- [x] 16. ページネーション (`.pagination`, `.page`)
...
- [x] 30. 未使用スタイル削除 (`.sub-column .pawprint-info`, `.sub-column .pawprint-memo`)
- [x] 31. 共通スタイル (`a`, `textarea`等)

## Tailwind CSSへ移行できないスタイル（残すもの）

以下のスタイルは、技術的な理由によりTailwind CSSへの移行が不可能なため、カスタムCSSとして残します。

### 画像のblur背景効果 (`::before` 疑似要素)

汎用クラス `.image-blur-bg::before` を使用しています。このクラスは、`::before` 疑似要素で `background: inherit` を使用して、背景画像をぼかして背景として表示し、その上に綺麗な画像を重ねる効果を実現しています。

**使用箇所：**
- チャンネルプロフィール画像 (`app/views/channels/show.html.erb`)
- チャンネルカード画像 (`app/views/channels/_flex_cards.html.erb`, `app/views/channels/_grid_cards.html.erb`)
- アイテムカード画像 (`app/views/items/_cards.html.erb`)
- Pawprintアイテム画像 (`app/views/pawprints/_blocks.html.erb`, `app/views/pawprints/_lines.html.erb`)

**残す理由：**
- Tailwind CSSの `before:bg-[inherit]` は `background-color: inherit` にしかならない
- `background-image` を継承するには、標準CSSの `background: inherit` が必要
- Tailwind CSSだけでは表現不可能
