...
2. **移行前の表示確認**
   ...
   - **各サイズで画像スクリーンショットを撮影して保存**
     - PC (1280x800): `take_screenshot(filePath='tmp/screenshots/{component}-pc-before.png')`
     - タブレット (768x800): `resize_page(width=768, height=800)` → `take_screenshot(filePath='tmp/screenshots/{component}-tablet-before.png')`
     - スマートフォン (375x667): `resize_page(width=375, height=667)` → `take_screenshot(filePath='tmp/screenshots/{component}-mobile-before.png')`
   - **表示確認チェックリスト**:
     - [ ] コンポーネントが正しく表示されている
     - [ ] レイアウトが崩れていない
     - [ ] 各デバイスサイズで適切に表示される
...
4. **移行後の表示確認**
   ...
   - **各サイズで画像スクリーンショットを撮影**
     - PC (1280x800): `take_screenshot(filePath='tmp/screenshots/{component}-pc-after.png')`
     - タブレット (768x800): `take_screenshot(filePath='tmp/screenshots/{component}-tablet-after.png')`
     - スマートフォン (375x667): `take_screenshot(filePath='tmp/screenshots/{component}-mobile-after.png')`
   - **移行前のスクリーンショットと目視で比較**
     - 画像ファイルを並べて確認
   - **表示確認チェックリスト**:
     - [ ] レイアウトが崩れていない（移行前と同じ）
     - [ ] 余白・マージンが適切（移行前と同じ）
     - [ ] フォントサイズ・色が同じ
     - [ ] ボタン・リンクのスタイルが適用されている
     - [ ] ホバー状態のスタイルが正しい
     - [ ] PC・タブレット・スマートフォンすべてで表示が正しい
