...
4. 結果を一覧表示した上で、以下のルールで処理:
   - **patch/minor + CI全てpass**: マージ前にコメントを残してから `gh pr merge <PR番号> --merge` で自動マージ
     - コメント: `gh pr comment <PR番号> --body "Merged by Claude Code /dependabot-merge skill."`
   - **major + CI全てpass**: 内容を表示し、ユーザーに確認を取ってからマージ（マージする場合は同様にコメントを残す）
