...
3. 各PRについて以下を判定:
   - PRタイトルからバージョンアップの種類を判定（semverのpatch/minor/major）
     - "from X.Y.Z to X.Y.W" のようなパターンでmajor/minor/patchを識別（判定できない場合はmajorとして扱う）
   - `statusCheckRollup`フィールドからCIの状態を確認
4. 結果を一覧表示した上で、以下のルールで処理:
   - **patch/minor + CI全てpass**: `gh pr merge <PR番号> --merge` で自動マージし、マージ成功を確認してからコメントを残す
     - マージ後に `gh pr view <PR番号> --json state` などで`MERGED`になったことを確認
     - 確認できたら: `gh pr comment <PR番号> --body "Merged by Claude Code /dependabot-merge skill."`
   - **major + CI全てpass**: 内容を表示し、ユーザーに確認を取ってからマージ（マージ成功を確認してから同様にコメントを残す）
